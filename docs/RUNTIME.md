...
This sits in five places, covering every guest store: the inline `vm_write8/16/32/64` in `ppu_memory.h` (HLE modules, including the SPURS ones that write the management structures), the out-of-line `vm_write8/16/32/64` in `ppu_loader.cpp` (recompiled guest code), the successful commit inside `ppu_stwcx`/`ppu_stdcx` and `ppu_stwcx32`/`ppu_stdcx64`, and the block writers `vm_memcpy_to`/`vm_memset`, which scan the 128-byte lines they cover and, when any is reserved, do the copy under the lock-line lock and notify each reserved line once, the way the SPU DMA path below does for a PUT. The lifter emission is unchanged: it already routes stores and store-conditionals through those functions.
