...
| `win32_backtrace.h` | `RtlCaptureStackBackTrace`, `GetModuleHandleA/W`, `GetModuleHandleExA/W`, `GetModuleFileNameA/W`, and the dbghelp names a crash report uses (`SymInitialize`, `SymFromAddr`, `SymCleanup`), over `backtrace(3)` and `dladdr(3)` | passthrough |
...
| `GetModuleHandleW`, `GetModuleHandleExW`, `GetModuleFileNameW` | the same `dladdr` the A forms use; the name argument is never read, since the only questions asked are "this image" and "the image at this address" | same | a real wide conversion -- `GetModuleFileNameW` widens one byte to one `wchar_t`, so a non-ASCII path is mojibake rather than truncated |
| `InitOnceExecuteOnce` | a mutex, a condition variable and a state behind the `INIT_ONCE` slot, so the callback's parameter, its context and its `FALSE` all survive | same | nothing: the failure-retry and context halves are the reason it is not `pthread_once` |
| `CreateDirectoryA` | `mkdir(path, 0777)`, with `EEXIST` reported as `ERROR_ALREADY_EXISTS` so an unconditional create still tells that apart from a permission failure | same | the security descriptor; separator translation -- a backslash is an ordinary filename character here |
| `SetProcessInformation` | accepted and ignored: Darwin's QoS is set per thread, by `SetThreadPriority` | accepted and ignored: there is no throttle to opt out of | `ProcessPowerThrottling` and every other class. The timer period, priority class and thread priority are separate calls and are real |
...
| Difference | Where |
|---|---|
| **Weak memory model.** x86 is TSO; arm64 reorders freely. Lifted guest code carries its own fences: the PPU `sync`/`ptesync`/`lwsync`/`eieio`/`isync` lower through `PPU_FENCE` in the lifter's header preamble to `seq_cst`/`seq_cst`/`acq_rel`/`release`/`acquire`, and the SPU `sync`/`dsync`/`syncc` to `spu_arch_fence`. That leaves the audit target: host runtime code that used `volatile` or a plain store to publish state. Every "data stores, then flag store" needs a release/acquire pair; the shim's Interlocked ops are sequentially consistent and `ReadAcquire`/`WriteRelease` are there for the flag case. | `tools/ppu_lifter.py`, `runtime/spu/spu_helpers.h`, `runtime/platform/win32_compat.h`, per-module |
| **A guest barrier is not free to drop.** The PPU barriers were no-ops until 2026-09; on x86 that was invisible, because TSO already gave the guest the order it asked for. It is exactly the "data stores, then flag store" rule above, written by the game rather than by us: gcm's SDK code separates the command words from the FIFO put pointer with `eieio` or `lwsync`, and without the fence arm64 may publish the pointer first and let the consumer read past the producer. Cheap in practice: `dmb ish` for three of the five and `dmb ishld` for `isync`, ~0.5 ns each here, and a real title's EBOOT holds a few dozen of them (46 in Yakuza: Dead Souls, against 4.98 M instructions). On x86-64 only `sync`/`ptesync` emit anything. | `tools/ppu_lifter.py`, `docs/PPU_RECOMP.md` |
